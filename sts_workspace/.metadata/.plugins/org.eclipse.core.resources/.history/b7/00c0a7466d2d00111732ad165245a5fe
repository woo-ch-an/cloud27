	...
	<form method="post" action="/update/${article.id}">
		<div class="gird write">
			<label for="subject">제목</label> <input type="text" id="subject"
				name="subject" placeholder="Write your title" value="${article.subject}" /> <label for="email" >이메일</label>
			<input id="email" type="email" name="email"
				placeholder="Enter the Email" value="${article.email}" /> <label for="content" >내용</label>
