...
//			// Super 관리자 또는 권한을 갖고 있ㄴㄴㅈ직 검 ㅁㄴ ㅜㄴ엄ㅇ눟ㅁㅇ라 훔ㄴ어후 두너우라ㅓ쟏ㅎ머ㅓ두훅궇 ㅁ롷ㄴㅇㅅㅇㅅ에ㅐ서 조노홍홍ㄴㄱ오상서 암누어룯허묻주ㅏㅓ맏거ㅜㅎ
//			
//			List<String> roles = loginUser.getRoles();
//			boolean isAdmin = roles.contains("RL-20260414-000001") || roles.contains("RL-20260414-000002");
//			
//			if (!isAdmin && !loginUser.getEmail().equals(board.getEmail())) {
//				throw new HelloSpringException("잘못된 접근입니다.", "errors/403");
//			}
		}
		// 조회한 게시글을 반환.
	...
	public boolean updateBoardByArticleId(UpdateVO updateVO) {
		...
		// 게시글불러와서 권한검사후 ㅇ{엥 ㅗ이처리
		String loginUserEmail = AuthUtils.getUsername();
		boolean isAdminAccount = AuthUtils.hasAnyRole("RL-20260414-000001" ,"RL-20260414-000002");
		
		if(!isAdminAccount && !loginUserEmail.equals(updateVO.getEmail())) {
			if (ServletUtils.isApiRequest()) {
				throw new HelloSpringApiException("접근권한 오류", HttpStatus.FORBIDDEN.value(), "잘못된 접근입니다.");
			}
			else {
	...
	public boolean deleteAllBoard() {
		...
		if (deleteCount > 0) {
			...
			for (String filePath: attachFilePaths) {
				...
				fileDeleteCount += this.filesDao.deleteFileByFileGroupId(filePath);
			}
		}
